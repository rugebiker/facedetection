...
//		fseek(in_file, position-1, SEEK_SET);
	...
	// Read info (row, col, maxgray)

	fgets (mystring , 20, in_file);
	pch = (char *)strtok(mystring," ");
//	strcpy(image->widthString, pch);
	image->width = atoi(pch);
//	image->width = myatoi(pch);
	pch = (char *)strtok(NULL," ");
//	strcpy(image->heightString, pch);
	image->height = atoi(pch);
//	image->height = myatoi(pch);
	fgets (mystring , 5, in_file);
//	strcpy(image->maxgreyString, mystring);
	image->maxgrey = atoi(mystring);
//	image->maxgrey = myatoi(mystring);
